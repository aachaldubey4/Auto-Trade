    ...
    : IMarketDataProvider
{
    ...
    private static DateTime _yahooCooldownUntilUtc = DateTime.MinValue;
    private static readonly TimeSpan YahooCooldown = TimeSpan.FromMinutes(5);
    ...
    public async Task<List<OhlcData>> GetHistoricalDataAsync(string symbol, int days)
    {
        ...
        {
            if (config.MarketData.UseSyntheticFallback)
            {
                logger.LogWarning(ex, "Historical data unavailable for {Symbol}, returning SYNTHETIC series (demo mode)", symbol);
                var synthetic = GenerateSyntheticHistorical(symbol, days);
                cache.Set(cacheKey, synthetic, new MemoryCacheEntryOptions().SetAbsoluteExpiration(TimeSpan.FromMinutes(5)));
                return synthetic;
            }
    ...
    {
        if (DateTime.UtcNow < _yahooCooldownUntilUtc)
        {
            var remaining = _yahooCooldownUntilUtc - DateTime.UtcNow;
            throw new InvalidOperationException(
                $"Yahoo Finance in 429 cooldown for {remaining.TotalSeconds:F0}s; skipping historical fetch for {symbol}");
        }
        try
        {
        ...
        catch (Exception ex)
        {
            ...
            {
                _yahooCooldownUntilUtc = DateTime.UtcNow + YahooCooldown;
                logger.LogWarning("Yahoo historical 429 detected; cooling down for {Minutes} min", YahooCooldown.TotalMinutes);
            }
    ...
    private async Task<MarketQuote> FetchQuoteWithFallbackAsync(string symbol)
    {
        ...
        catch (Exception yEx)
        {
            ...
            if (config.MarketData.UseSyntheticFallback)
            {
                var synthetic = GenerateSyntheticQuote(symbol);
                logger.LogWarning("Using SYNTHETIC fallback quote for {Symbol}: {Price} (demo mode)",
    ...
    private static readonly Dictionary<string, decimal> SyntheticBasePrices = new(StringComparer.OrdinalIgnoreCase)
    {
        ["RELIANCE"] = 2950m, ["TCS"] = 4120m, ["INFY"] = 1680m, ["HDFCBANK"] = 1685m,
        ["ICICIBANK"] = 1280m, ["SBIN"] = 825m, ["HINDUNILVR"] = 2380m, ["ITC"] = 445m,
        ["KOTAKBANK"] = 1790m, ["BHARTIARTL"] = 1620m, ["LT"] = 3580m, ["AXISBANK"] = 1170m,
        ["MARUTI"] = 12450m, ["BAJFINANCE"] = 7320m, ["ASIANPAINT"] = 2890m, ["WIPRO"] = 545m,
        ["HCLTECH"] = 1820m, ["SUNPHARMA"] = 1750m, ["TITAN"] = 3450m, ["NESTLEIND"] = 2480m,
        ["TATAMOTORS"] = 980m, ["TATASTEEL"] = 152m, ["ULTRACEMCO"] = 11250m, ["POWERGRID"] = 320m,
        ["NTPC"] = 365m, ["ONGC"] = 275m, ["COALINDIA"] = 480m, ["JSWSTEEL"] = 935m,
        ["ADANIPORTS"] = 1340m, ["BAJAJFINSV"] = 1620m, ["DRREDDY"] = 6280m, ["CIPLA"] = 1520m
    };

    private static decimal DeriveSyntheticBase(string symbol)
    {
        var h = 0;
        foreach (var c in symbol.ToUpperInvariant()) h = h * 31 + c;
        var abs = Math.Abs(h);
        var bucket = abs % 4;
        var jitter = (abs / 7) % 1000;
    ...
    private MarketQuote GenerateSyntheticQuote(string symbol)
    {
        var basePrice = SyntheticBasePrices.TryGetValue(symbol, out var bp) ? bp : DeriveSyntheticBase(symbol);
        var today = DateTime.UtcNow.Date;
        var daySeed = HashCode.Combine(symbol.ToUpperInvariant(), today.Ticks);
        var dayRng = new Random(daySeed);
        var dayChange = (decimal)((dayRng.NextDouble() - 0.5) * 0.04); // ±2%
        var open = Math.Round(basePrice * (1 + dayChange), 2);

        var slot = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, _istTimeZone);
        var intradaySeed = HashCode.Combine(daySeed, slot.Hour, slot.Minute / 5);
        var rng = new Random(intradaySeed);
        var driftPct = (decimal)((rng.NextDouble() - 0.45) * 0.025);
        var last = Math.Round(open * (1 + driftPct), 2);
        var high = Math.Round(Math.Max(open, last) * (1 + (decimal)(rng.NextDouble() * 0.008)), 2);
        var low = Math.Round(Math.Min(open, last) * (1 - (decimal)(rng.NextDouble() * 0.008)), 2);
        var prevClose = Math.Round(basePrice, 2);
        var volume = 500_000L + (long)(rng.NextDouble() * 4_500_000);
    ...
    private List<OhlcData> GenerateSyntheticHistorical(string symbol, int days)
    {
        var basePrice = SyntheticBasePrices.TryGetValue(symbol, out var bp) ? bp : DeriveSyntheticBase(symbol);
        ...
        while (dates.Count < days)
        {
            ...
            if (_marketHolidays.Contains(cursor)) continue;
            dates.Add(cursor);
        }
        dates.Reverse();

        var walk = basePrice * 0.95m;
        var result = new List<OhlcData>(days);
        foreach (var dt in dates)
        {
            var rng = new Random(HashCode.Combine(symbol.ToUpperInvariant(), dt.Ticks));
            var changePct = (decimal)((rng.NextDouble() - 0.48) * 0.03);
            var open = Math.Round(walk, 2);
            var close = Math.Round(open * (1 + changePct), 2);
            var high = Math.Round(Math.Max(open, close) * (1 + (decimal)(rng.NextDouble() * 0.012)), 2);
            var low = Math.Round(Math.Min(open, close) * (1 - (decimal)(rng.NextDouble() * 0.012)), 2);
            var vol = 500_000L + (long)(rng.NextDouble() * 3_500_000);
            result.Add(new OhlcData { Date = dt, Open = open, High = high, Low = low, Close = close, Volume = vol });
            walk = close;
        }
        return result;
    }
    ...
    private async Task<MarketQuote> FetchFromYahooFinanceWithRetryAsync(string symbol)
    {
        if (DateTime.UtcNow < _yahooCooldownUntilUtc)
        {
            var remaining = _yahooCooldownUntilUtc - DateTime.UtcNow;
            throw new InvalidOperationException(
                $"Yahoo Finance in 429 cooldown for {remaining.TotalSeconds:F0}s; skipping to next fallback");
        }
        ...
        for (var attempt = 0; attempt <= retries; attempt++)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    _yahooCooldownUntilUtc = DateTime.UtcNow + YahooCooldown;
                    logger.LogWarning("Yahoo 429 detected; cooling down for {Minutes} min", YahooCooldown.TotalMinutes);
                    break;
                }
                if (attempt == retries) break;
